...
var streamInfo = {
  ...
  streamName: WOWZA_STREAM_NAME,
  sessionId: WOWZA_SESSION_ID_EMPTY,
};

var WOWZA_STREAM_NAME = null,
  connections = {},
  localStream = null;
...
const initializeConnection = (partnerClientId) => {
  ...
  connection.onicecandidate = (evt) =>
    callbackIceCandidate(evt, connection, partnerClientId); // ICE Candidate Callback
...
wsconn.on("updateUserList", (userList) => {
  ...
  $.each(userList, function (index) {
    ...
    if (!userIcon) {
      userIcon = userList[index].inCall
        ? "icon-smartphone-1"
        : "icon-smartphone-1";
    }
    ...
    var listString =
      '<li class="list-group-item user" data-cid=' +
      userList[index].connectionId +
      " data-username=" +
      userList[index].username +
      ">";
    listString +=
      '<a href="#"><div class="username"> ' +
      userList[index].username +
      "</div>";
    listString +=
      '<span class="helper ' +
...
const errorHandler = (error) => {
  if (error.message)
    alertify.alert(
      "<h4>Error Occurred</h4></br>Error Info: " + JSON.stringify(error.message)
    );
  else
    alertify.alert(
      "<h4>Error Occurred</h4></br>Error Info: " + JSON.stringify(error)
    );
